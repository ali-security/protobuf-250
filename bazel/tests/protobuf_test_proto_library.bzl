...
def protobuf_test_proto_library(**kwattrs):
    ...
    # Bazel 7's proto_library rule does not support option_deps, so we handle it by putting it in deps instead.
    if "option_deps" in kwargs and not hasattr(proto_library, "option_deps"):
